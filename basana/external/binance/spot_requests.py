...
"""
【中文说明】Binance现货订单请求模块
【功能描述】提供Binance现货交易的各种订单类型封装和创建功能
【使用场景】用于创建和管理现货账户的市价单、限价单、止损限价单和OCO订单
【核心功能】
...
【注意事项】所有订单都支持现货交易的标准参数，不支持保证金特有的side_effect_type
"""
...
class ExchangeOrder(metaclass=abc.ABCMeta):
    """
    【中文说明】交易所订单抽象基类
    【功能描述】定义现货订单的通用接口和基础属性
    【使用场景】作为所有现货订单类型的基类，提供统一的订单创建接口
    【继承关系】所有具体订单类都继承自此类
    """
    def __init__(
...
class MarketOrder(ExchangeOrder):
    """
    【中文说明】市价单类
    【功能描述】封装Binance现货市价单的创建和参数管理
    【使用场景】用于以当前市场价格立即成交的现货订单
...
class LimitOrder(ExchangeOrder):
    """
    【中文说明】限价单类
    【功能描述】封装Binance现货限价单的创建和参数管理
    【使用场景】用于以指定价格或更好价格成交的现货订单
    【继承关系】继承自ExchangeOrder
    【订单特性】支持设置价格和订单时效
    """
    def __init__(
...
class StopLimitOrder(ExchangeOrder):
    """
    【中文说明】止损限价单类
    【功能描述】封装Binance现货止损限价单的创建和参数管理
    【使用场景】用于在价格达到止损价时以限价方式成交的现货订单
    【继承关系】继承自ExchangeOrder
...
class OCOOrder:
    """
    【中文说明】OCO订单类
    【功能描述】封装Binance现货OCO订单（一个取消另一个）的创建和参数管理
